        ...
        <form
            ...
            <div className={` w-full flex-col flex justify-center items-center gap-4`}>
                <div className={` flex flex-col gap-2 justify-center w-full`}>
                    ...
                    <input className={`border-gray-500`} type={"password"} value={currentPassword} onChange={event => {
                        setCurrentPassword(event.target.value)
                    }}/>
                </div>
                <div className={` flex flex-col gap-2 justify-center w-full`}>
                    <label>New Password</label>
                    <input className={`border-gray-500`} type={"text"} value={newPassword} onChange={event => {
                        setNewPassword(event.target.value)
                    }}/>
                </div>
                <div className={` flex flex-col gap-2 justify-center w-full`}>
                    <label>Confirm New Password</label>
                    <input className={`border-gray-500`} type={"password"} value={confirmNewPassword}
                           onChange={event => {
                               setConfirmNewPassword(event.target.value)
                           }}/>
                </div>
            </div>
            <button type={"submit"} onClick={changePasswordFunc}>Save</button>
        </form>
    </div>
}
